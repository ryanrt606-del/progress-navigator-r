...
import {
  ChevronLeft, Plus, RotateCcw, Trash2, Sparkles, Calendar, ListChecks, Pencil, Check, X,
} from "lucide-react";
...
export function PlanView({
  ...
  return (
    <div className="h-full flex flex-col bg-background overflow-hidden">
      {/* ── Sticky header ── */}
      <div className="shrink-0 px-4 pt-14 pb-4 border-b border-border/40 bg-background/90 backdrop-blur-xl">
        <div className="max-w-xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-foreground text-lg truncate leading-tight">{plan.title}</h1>
        ...
        <div className="max-w-xl mx-auto px-4 py-5 pb-10">
          ...
          >
            ...
            <div className="relative">
              <div className="flex items-end justify-between mb-5">
                <div>
                  <p className="text-white/60 text-xs font-semibold uppercase tracking-widest mb-1">Completion</p>
                  <motion.p
                ...
                <div className="text-right">
                  <p className="text-white/50 text-xs mb-1 uppercase tracking-wider">Tasks</p>
                  <p className="text-white text-2xl font-bold leading-none">
                    {done}
                    <span className="text-white/40 text-base font-normal"> / {plan.steps.length}</span>
                  </p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <motion.button
              whileTap={{ scale: 0.95 }}
              onClick={onResetPlan}
              className="flex items-center justify-center gap-2 rounded-2xl py-3 text-sm font-medium text-muted-foreground card-glass hover:border-border/60 hover:text-foreground transition-colors"
            >
              <RotateCcw size={14} />
              Reset
            </motion.button>
